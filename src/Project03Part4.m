...
%Program: Solution for AAS, T1.2019, Project3.Part4

function Project03Part4
    ...
    for i=1:IMU.N-1
        ...
        Xe = Xe + Dt*[Velocity(i)*cos(Xe(3)); Velocity(i)*sin(Xe(3)); YawRate(i)-Xe(4); 0];
        
        J = [1,0,-Dt*Velocity(i)*sin(Xe(3)),   0; 
             0,1, Dt*Velocity(i)*cos(Xe(3)),   0; 
             0,0,             1            , -Dt;
             0,0,             0            ,   1];
        Fu = [Dt*cos(Xe(3)),  0; 
              Dt*sin(Xe(3)),  0; 
...
function m = IniPlots()
    ...
    ax = gca;
    ax.GridColor = [0.93, 0.93, 0.93];
    m.bias = animatedline('color',[0.5, 0.85, 1.0],'MaximumNumPoints',4000);
    axis tight;
